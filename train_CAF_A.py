...
def train(config):
    logger = logging.getLogger("CAF-A.trainer")
    ...
    # for discriminator
    optimizer_d = optimizer_config["type"](ad_net.get_parameters(),
                                           **(optimizer_config["optim_params"]))
    param_d_lr = []
    for param_group in optimizer_d.param_groups:
        param_d_lr.append(param_group["lr"])
    ...
    for i in tqdm(range(config["num_iterations"])):
        if i % config["test_interval"] == 0:
            ...
            if config["dataset"] == "visda2017":
                temp_acc, avg_for_class = test(dset_loaders,
                                               base_network,
                                               classifier1,
                                               classifier2,
                ...
                logger.info("iter: {:05d}, precision: {:.5f},\tbest_class_acc: {:.5f}\tall_acc:{:.5f}\ttime: {:.2f}".format(
                    i, avg_for_class, best_acc, temp_acc, time.time() - start))
            else:
                temp_acc = test(dset_loaders,
                                base_network,
                                classifier1,
                                classifier2,
                                test_10crop=prep_config["test_10crop"],
                ...
                logger.info("iter: {:05d}, precision: {:.5f},\tbest_acc:{:.5f}\ttime: {:.2f}".format(
                    i, temp_acc, best_acc, time.time() - start))
        ...
        optimizer_d = lr_scheduler(optimizer_d, i, **schedule_param)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Collaborative Alignment Framework - Adversarial Learning')
    ...
    parser.add_argument('--output_dir', type=str, default='result/CAF-A',
    ...
    config["output_path"] = os.path.join(args.output_dir, 'CAF-A', args.dset + f'_seed-{args.SEED}')
    ...
    logger = setup_logger("CAF-A", config["output_path"], 0, filename=file_name)
